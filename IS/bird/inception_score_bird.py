...
tf.app.flags.DEFINE_string('image_folder', 'outout_tmp_models/bird_sloss01/64/models/netG_550', """Path where to load the fake images """)

tf.app.flags.DEFINE_integer('num_classes', 50,      
                            """Number of classes """)
...
def get_inception_score(sess, images, pred_op):
    ...
    assert(type(images[50]) == np.ndarray)
    assert(len(images[50].shape) == 3)
    assert(np.max(images[50]) > 10)
    assert(np.min(images[50]) >= 0.0)
    bs = FLAGS.batch_size
    preds = []
        ...
        for name in files:
            if name.rfind('jpg') != -1 or name.rfind('png') != -1:
                ...
                if os.path.isfile(filename):
                    ...
                    images.append(img)
    print('images', len(images), images[50].shape)
    print('1 images',images[1].shape) ##############
    print('50 images',images[50].shape) ############
    print('100 images',images[100].shape) ############
    return images
